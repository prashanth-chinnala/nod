...
| MuseTalk v1.5 | Here |
|---|---|
| `load_all_model(unet_path, vae_type, unet_config, device)` | `load()`, absolute paths |
...
Larger batches use the device better and cost first-frame latency, since no frame in a batch
emits until all of them are done. 8 is MuseTalk's own default. `NOT YET MEASURED` on this
hardware -- tune once, with numbers, rather than guessing repeatedly.
"""
...
class TorchMuseTalkBackend:
    ...
    def load(self) -> None:
        ...
        vae, unet, pe = load_all_model(
            unet_model_path=str(self.model_root / "musetalkV15" / "unet.pth"),
            vae_type="sd-vae",
            unet_config=str(self.model_root / "musetalkV15" / "musetalk.json"),
            device=torch.device(self.device),
        )
        # `load_all_model` builds the VAE from `models/<vae_type>` relative to the working
        # directory, so it is rebuilt here against an absolute path. Cheap: the weights are
        # already in the page cache from the first attempt.
        from musetalk.models.vae import VAE

        vae = VAE(model_path=str(self.model_root / "sd-vae"))

        dtype = unet.model.dtype
        vae.vae = vae.vae.to(device=self.device, dtype=dtype)
        unet.model = unet.model.to(device=self.device)
        pe = pe.to(device=self.device, dtype=dtype)
