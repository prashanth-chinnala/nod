...
class BrowserSession:
    ...
    def __init__(self, socket: WebSocket, *, session_id: str | None = None) -> None:
        ...
        if DELIVERY == "worker" and session_id:
            ...
            if not self.worker_reason:
                self._worker = WorkerAudioTransport(f"session-{session_id}")
                self._transport = _Tee(self._worker, self._transport)
        elif session_id and _livekit_available():
...
def _worker_reason() -> str:
    ...
    return WorkerAudioTransport.available()
